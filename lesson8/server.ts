...
import { z } from "zod";
import { loadMoviesAsync, addMovie, listMovies, markAsWatched, type Movies, clearMovie, loadMoviesPaginatedAsync } from "./movies";
...
app.patch("/movies/:id", validateParams(paramsSchema), validate(updateMovieSchema), async (request, response) => {
  ...
  await markAsWatched(foundMovie.id, watched);
...
if (process.env.NODE_ENV !== 'test') {
  app.listen(port, () => {
    console.log(`Server is running on http://localhost:${port} and is using data base ${process.env.DB_FILE}`);});
